    ...
    return (
        <div className="min-h-screen px-6 py-12 max-w-5xl mx-auto">
            ...
            <div className="mb-8 animate-fade-up">
                <span className="text-xs text-white/40 uppercase tracking-widest">Observability</span>
                <h1 className="text-4xl font-black mt-1 mb-2 text-white">AI Prompt <span className="gradient-text">Logs</span></h1>
                <p className="text-white/50">Every AI call is logged — prompt, response, duration, and parsed output.</p>
            </div>

            {/* Filter */}
            <div className="flex items-center gap-3 mb-6 animate-fade-up" style={{ animationDelay: "60ms" }}>
                {["", "categorize", "proposal", "impact", "chat"].map((m) => (
                    <button
                        key={m}
                        onClick={() => setModule(m)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${module === m
                ...
                <button onClick={fetchLogs} className="ml-auto text-xs text-white/40 hover:text-white/70 transition-colors">
                    ↻ Refresh
                </button>
            </div>
            ...
            {loading ? (
                ...
                <div className="space-y-3">
                    {logs.map((log) => (
                        <div key={log.id} className="glass rounded-xl border border-white/8 overflow-hidden">
                            {/* Log header */}
                            <button
                                onClick={() => setExpanded(expanded === log.id ? null : log.id)}
                                className="w-full flex items-center gap-3 px-5 py-3.5 hover:bg-white/3 transition-colors text-left"
                            >
                                <span className={`text-xs px-2 py-0.5 rounded border font-mono ${MODULE_COLORS[log.module] ?? "text-white/50"}`}>
                                    {log.module}
                                </span>
                                ...
                                {log.durationMs && (
                                    <span className="text-xs text-white/30">{log.durationMs}ms</span>
                                )}
                                <span className="text-xs text-white/30 ml-auto">
                                    {new Date(log.createdAt).toLocaleString("en-IN")}
                                </span>
                                <span className="text-white/30 text-xs">{expanded === log.id ? "▲" : "▼"}</span>
                            </button>
                                ...
                                <div className="border-t border-white/8 p-5 space-y-4">
                                    ...
                                    <div>
                                        ...
                                        <pre className="text-xs text-white/60 font-mono bg-black/30 p-3 rounded-lg overflow-x-auto max-h-40 whitespace-pre-wrap">
                                            {log.prompt.slice(0, 800)}{log.prompt.length > 800 ? "..." : ""}
                                        </pre>
                                    </div>
                                    <div>
                                        <p className="text-xs text-white/40 mb-1.5">RAW RESPONSE</p>
                                        <pre className="text-xs text-white/60 font-mono bg-black/30 p-3 rounded-lg overflow-x-auto max-h-40 whitespace-pre-wrap">
                                            {log.response.slice(0, 600)}{log.response.length > 600 ? "..." : ""}
                                        </pre>
                                    </div>
                                    {log.parsedJson && (
                                        <div>
                                            <p className="text-xs text-white/40 mb-1.5">PARSED OUTPUT</p>
